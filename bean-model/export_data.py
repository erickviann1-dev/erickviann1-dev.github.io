...
def main():
    ...
    research_rigor = [
        ...
        },
        {
            ...
            "label_zh": "测试窗口",
            "body_en":  "2025+ · 15,073 daily observations · held out completely until the final rule set was selected. Headline metrics are reported with their own stated validation windows; the 80-day 88.5% result is from the completed-horizon sub-window, while the 1,199-signal scan describes the broader DB3-500 background.",
            "body_zh":  "2025+ · 共 15,073 条日线样本 · 在最终规则集选定之前从未被模型看过。页面头条指标均按各自标注的验证窗口报告；80 日 88.5% 来自完整持有期子窗口，1,199 笔信号则对应更宽的 DB3-500 背景扫描。",
        },
        {
            "label_en": "Transaction costs",
            "label_zh": "交易成本",
            "body_en":  "Headline win rates are gross of execution costs. For a realistic retail-broker scenario, deduct approximately 10–15 bps round-trip (commission + slippage + 5 bp stamp duty on sells). For institutional execution, approximately 2–5 bps. Costs affect net returns more directly than headline win rate; net-return sensitivity should be reported separately.",
            "body_zh":  "头条胜率为不计成本的毛值。零售券商口径下，扣除约 10-15 个基点的往返成本（佣金 + 滑点 + 卖出 0.05% 印花税）。机构执行约 2-5 bps。交易成本更直接影响净收益，而不是简单线性改写胜率；净收益敏感性应单独报告。",
        },
        {
